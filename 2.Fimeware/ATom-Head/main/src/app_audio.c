...
esp_err_t aplay_mp3(const char *path)
{
    ...
    read_buf = heap_caps_malloc(MAINBUF_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    ESP_GOTO_ON_FALSE(NULL != read_buf, ESP_ERR_NO_MEM, clean_up,
                      TAG, "Failed allocate read buffer");

    output = heap_caps_malloc(1152 * sizeof(int16_t) * 2, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
...
clean_up:
    ...
    ESP_LOGI(TAG, "Clean up resources\n");
    return ret;
}
