...
static esp_err_t audio_mp3_load(const char *filepath, size_t *file_len)
{
    ...
    do
    {
        ...
        {
            memcpy((void *)(instance.mp3_player_buffer + total_cnt), buffer, len);
            total_cnt += len;
        }
    }
    while (1);
...
static int find_Index(const char *file_name)
{
    size_t index;
...
static const char *player_get_name_from_index(size_t index)
{
...
esp_err_t audio_player_play_name(const char *file_name)
{
    ...
    strcpy(full_name, instance.config.base_path);
    strcat(full_name, "/");
    strcat(full_name, player_get_name_from_index(index));
    printf("%s\n", full_name);

    size_t len;
    audio_mp3_load(full_name, &len);
    if (len)
        audio_player_play(instance.mp3_player_buffer, len);
...
esp_err_t APP_Player_run(audio_player_config_t config)
{
    int ret = ESP_OK;
    BaseType_t task_val;

    char *base_path = (char *) config.base_path;
    fm_init(base_path);
    /* Scan audio file */
