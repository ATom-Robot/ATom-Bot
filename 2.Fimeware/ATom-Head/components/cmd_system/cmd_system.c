...
static void register_iram(void);
...
/* 'heap' command prints minumum heap size */
static int iram_size(int argc, char **argv)
{
    size_t iram_max = heap_caps_get_largest_free_block(MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
    size_t iram_min = heap_caps_get_minimum_free_size(MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);

    ESP_LOGI(TAG, "iram max size: %u", iram_max);
    ESP_LOGI(TAG, "iram min size: %u", iram_min);
...
static void register_iram(void)
{
    ...
    ESP_ERROR_CHECK(esp_console_cmd_register(&iram_cmd));
}
