...
const int resistor_divider_coefficient = (100 + 100) / 100;
...
float adcx_get_chx_value_average(ADC_HandleTypeDef *ADCx, uint32_t ch, uint8_t times)
{
    ...
    {
        temp_val += adcx_get_chx_value(ADCx, ADC_CHANNEL_15);;
    }
...
void init_vrefint_reciprocal(void)
{
    ...
    LOG_D("total_adc = %f", total_adc);
    LOG_D("VREF = %f", (total_adc / 1000) * (3.3 / 4096));
    voltage_vrefint_proportion = times * vrefint / total_adc;
    LOG_D("voltage_vrefint_proportion = %f", voltage_vrefint_proportion);
...
分压的电阻值为100K和100K，由于(100K+100K)/100K=2，乘以这个值之后就可以得到电池的电压值。
*/
float get_battery_voltage(void)
{
    ...
    adcx = adcx_get_chx_value_average(&hadc1, ADC_CHANNEL_15, 100); //100次采样取平均值
