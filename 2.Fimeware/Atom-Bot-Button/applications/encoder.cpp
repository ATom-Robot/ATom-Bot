...
//	double vel = (double) diff_count / 820 / enc->sample_time * 1000 * 60;
//	rt_kprintf("%d.%d\n", (int)vel, (int)(vel * 10) % 10);
    return enc->cps;
}
...
rt_int16_t encoder_measure_rpm(encoder_t enc)
{
	...
	//rt_int16_t res_rpm = (float) encoder_measure_cps(enc) * 60 / (enc->pulse_revol * 30 * 4) * 10;
    return res_rpm;
}
