/*
 * Copyright (c) 2019, RT-Thread Development Team
 *
 * SPDX-License-Identifier: Apache-2.0
 *
 * Change Logs:
 * Date           Author       Notes
 * 2019-08-26     sogwms       The first version
 */

#include "motor.h"

#define DBG_SECTION_NAME  "motor"
#define DBG_LEVEL         DBG_LOG
#include <rtdbg.h>

motor_t motor_create(rt_size_t size)
{
    motor_t new_motor = (motor_t)rt_malloc(size);
    if (new_motor == RT_NULL)
    {
        LOG_E("Falied to allocate memory for new motor\n");
        return RT_NULL;
    }

    return new_motor;
}

rt_err_t motor_enable(motor_t mot)
{
    RT_ASSERT(mot != RT_NULL);

    // Enable PWM
    LOG_D("Enabling motor");
    mot->enable(mot);

    return RT_EOK;
}

rt_err_t motor_disable(motor_t mot)
{
    RT_ASSERT(mot != RT_NULL);

    // Disable PWM
    LOG_D("Disabling motor");
    mot->disable(mot);

    return RT_EOK;
}

rt_err_t motor_run_singal(motor_t mot, rt_int16_t thousands)
{
    RT_ASSERT(mot != RT_NULL);

    // Set speed (pwm) to desired value
    mot->set_speed(mot, thousands);

    return RT_EOK;
}

rt_err_t motor_run_double(motor_t mot1, motor_t mot2, rt_int16_t thousand1, rt_int16_t thousand2)
{
    RT_ASSERT(mot1 != RT_NULL);
    RT_ASSERT(mot2 != RT_NULL);

    // Set speed (pwm) to desired value
    mot1->set_speed(mot1, thousand1);
    mot2->set_speed(mot2, thousand2);

    return RT_EOK;
}

rt_err_t motor_stop(motor_t mot)
{
    RT_ASSERT(mot != RT_NULL);

    // Set Speed to 0
    motor_run_singal(mot, 0);

    return RT_EOK;
}

rt_err_t motor_destroy(motor_t mot)
{
    RT_ASSERT(mot != RT_NULL);

    // Disable first
    motor_disable(mot);

    // Free memory
    LOG_D("Free motor");

    rt_free(mot);

    return RT_EOK;
}

rt_err_t motor_reset(motor_t mot)
{
    RT_ASSERT(mot != RT_NULL);

    return mot->reset(mot);
}
