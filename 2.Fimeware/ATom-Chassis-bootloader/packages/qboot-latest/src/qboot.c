...
static bool qbt_release_sign_write(const char *part_name, fw_info_t *fw_info)
{
    ...
    if (fal_partition_erase(part, pos, sizeof(u32)) < 0)
    {
        LOG_E("Qboot erase release sign fail from %s partition.", part_name);
        return(false);
    }
